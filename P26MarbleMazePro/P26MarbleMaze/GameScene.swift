...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createBackground() {
        ...
        background.position = CGPoint(x: (frame.width / 2) - 25, y: frame.height / 2)
...
//        let sequence = SKAction.sequence([scaleDown, scaleUp])
    ...
    func playerCollided(with node: SKNode) {
        ...
        case "finish":
            ...
            let fadeFinsh = SKAction.fadeOut(withDuration: 0.15)
            let fadeFinshSequence = SKAction.sequence([fadeFinsh])
            finish.run(fadeFinshSequence)
