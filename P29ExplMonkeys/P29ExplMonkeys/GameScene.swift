...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createSnow() {
        snowFall = SKEmitterNode(fileNamed: "snow")
        snowFall?.zPosition = 1
        snowFall?.position = CGPoint(x: (view?.frame.width)! / 2, y: (view?.frame.height)! + 100)
        snowFall?.advanceSimulationTime(10)
        addChild(snowFall)
    }
    
    func launch(angle: Int, velocity: Int, wind: Int) {
        ...
        //  NOTE: To be sure everything works as intended, we're going to enable the usesPreciseCollisionDetection property for the banana's physics body. This works slower, but it's fine for occasional use.
        addChild(banana)
        ...
        if currentPlayer == 1 {
            ...
            let impulse = CGVector(dx: (cos(radians) * speed) + windSpeed, dy: sin(radians) * speed)
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        if firstNode.name == "banana" && secondNode.name == "player1" {
            viewController.updateScores(player: currentPlayer, playerScore: 1)
